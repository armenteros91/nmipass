...
public class NmiTransactionRequestDtoValidator : AbstractValidator<BaseTransactionRequestDto>
{
    public NmiTransactionRequestDtoValidator()
    {
        // Siempre requeridos
        RuleFor(x => x.TypeTransaction).NotEmpty().WithMessage("El tipo de transacción es requerido");
        ...
        When(x => x.PaymentType == "creditcard", () =>
        {
            RuleFor(x => x.CreditCardNumber).NotEmpty()
                .WithMessage("El número de tarjeta (ccnumber) es requerido para pagos con tarjeta");
            RuleFor(x => x.CreditCardExpiration)
                .NotEmpty()
                .WithMessage("La fecha de expiración (ccexp) es requerida para pagos con tarjeta");
        ...
        {
            RuleFor(x => x.CheckName).NotEmpty()
                .WithMessage("El nombre del titular (checkname) es requerido para pagos ACH");
            RuleFor(x => x.CheckAba).NotEmpty().WithMessage("El número ABA (checkaba) es requerido para pagos ACH");
            RuleFor(x => x.CheckAccount).NotEmpty()
                .WithMessage("El número de cuenta (checkaccount) es requerido para pagos ACH");
        });

        // Requiere campos para 3DS si se especifica cardholder_auth
        When(x => x.CardHolderAuth == "verified" || x.CardHolderAuth == "attempted", () =>
        {
            RuleFor(x => x.Cavv).NotEmpty()
                .WithMessage("El valor de autenticación (CAVV) es requerido para transacciones 3DS");
            RuleFor(x => x.Xid).NotEmpty()
                .WithMessage("El ID de autenticación (XID) es requerido para transacciones 3DS");
            RuleFor(x => x.ThreeDsVersion).NotEmpty().WithMessage("La versión de 3DS es requerida");
            RuleFor(x => x.DirectoryServerId).NotEmpty()
                .WithMessage("El ID del servidor de directorio es requerido para 3DS");
        });
        ...
        When(x => !string.IsNullOrEmpty(x.TransactionSessionId), () =>
        {
            RuleFor(x => x.TransactionSessionId)
                ...
                .WithMessage("TransactionSessionId debe tener exactamente 32 caracteres alfanuméricos");
        });
