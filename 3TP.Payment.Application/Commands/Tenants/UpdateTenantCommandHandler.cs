...
public class UpdateTenantCommandHandler : IRequestHandler<UpdateTenantCommand, Unit>
{
    ...
    public async Task<Unit> Handle(UpdateTenantCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Tenant with NmiTransactionRequestLogId {TenantId} not found for update", request.TenantId);
        ...
        {
            _logger.LogWarning(
                "Attempted to update tenant {TenantId} with company code {CompanyCode} that already exists",
                request.TenantId, request.CompanyCode);
        ...
        try
        {
            _unitOfWork.TenantRepository
                .Update(existingTenant); // Assuming Update is a synchronous method marking entity as modified
            await _unitOfWork.CommitAsync(cancellationToken);

            _logger.LogInformation(
                "Tenant {TenantId} updated successfully. New CompanyName: {CompanyName}, New CompanyCode: {CompanyCode}",
