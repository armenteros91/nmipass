...
public class TenantService : ITenantService
{
    ...
    public async Task<TenantApiKey> AddApiKeyAsync(Guid tenantId, string apiKeyValue, string? description, bool isActive)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("TenantService: Successfully added API key {ApiKeyId} to TenantId: {TenantId}", newApiKey.TenantApikeyId, tenantId);
            return newApiKey;
        }
        catch (InvalidTenantException ex) // Catch specific exception from tenant.AddApiKey (e.g. duplicate key)
        {
            _logger.LogWarning(ex, "TenantService: Error adding API key to tenant {TenantId}: {ErrorMessage}", tenantId, ex.Message);
            // Re-throw to be handled by presentation layer or global exception handler
            throw;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "TenantService: An unexpected error occurred while adding API key to tenant {TenantId}", tenantId);
