...
public class PaymentService: IPaymentService
{
    ...
    public async Task<NmiResponseDto> ProcessPaymentAsync(string apiKey, BaseTransactionRequestDto paymentRequest)
    {
        ...
        try
        {
            ...
            var secretValueResponse = await _awsSecretManagerService.GetSecretValueAsync(new GetSecretValueQuery { SecretId = secretIdForAws });
        ...
        requestLog.TenantId = tenant.Id; // Asociar con el ID interno del tenant
        ...
        {
            TenantId = tenant.Id, // Usar el ID interno del tenant (int)
            TraceId = paymentRequest.OrderId, // O OrderId si es el identificador único de la transacción desde el request
            Amount = paymentRequest.Amount,
            TypeTransactionsId = (int)paymentRequest.TypeTransaction, // Asegúrate que el enum se castea correctamente
            CreatedBy = "system" // O un identificador de usuario/sistema relevante
        });
